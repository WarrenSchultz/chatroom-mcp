...
| QUIC `timeout: no recent network activity` at startup, then recovery | Normal handshake retries while the connector picks edge nodes. | Ignore if it settles at four `Registered tunnel connection` lines and health is `healthy`. |
| Intermittent **502** to clients, but the origin is healthy and never restarted | The connector's QUIC links dropped and re-registered; for those seconds Cloudflare had no path to the origin, so it returns 502. Cloudflare's own 502 text says "origin overloaded", which is misleading — the origin may be untouched. | Check `docker logs chatroom-cloudflared` for `Connection terminated` and `no recent network activity`. If they recur, set `CLOUDFLARE_TUNNEL_PROTOCOL=http2` in `.env` and recreate the connector: QUIC rides UDP and some paths expire long-lived UDP flows, while http2 runs over TCP. Confirm with `docker logs chatroom-cloudflared \| grep -o 'protocol=[a-z0-9]*'`. |
